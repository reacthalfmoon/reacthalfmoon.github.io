...
const toast = (alertElement, timeShown) => {
    ...
    setTimeout(()=>{alertElement.classList.add('show')}, 250);
    const timeToFade = timeShown + 250;
    setTimeout(()=>{alertElement.classList.add('fade')}, timeToFade);
    const timeToDestroy = timeToFade + 500;
    setTimeout(() => {toastOut(alertElement)}, timeToDestroy)
}
...
const getStickyAlertElement = (title, content, dismissible, color, filled) => {
    ...
    if(dismissible){
        ...
        closeBtn.onclick = () => {toastOut(element)}
...
export default function stickyAlert ({title, content, dismissible, color, filled, timeShown}){
